  ...
  <span class="">
    <transition enter-active-class="zoomIn" leave-active-class="zoomOut">
      <div v-if="enJuego" class="animated row m-0 pb-3">
        ...
        />
        <MatrixLetras :sopa="sopa" :tam="tam" @cellSelected="selcell" />
...
import { frases } from "@/assets/js/Recursos";
  ...
  methods: {
    pausarJuego: function () {
      if (!this.sopaCompletada) {
        this.frase =
          frases[Math.floor(Math.random() * frases.length) % frases.length];
        this.enJuego = false;
        let pausa = Date.now();
    ...
    cambioTamano: async function (t) {
      this.pretam = t;
      this.tam = t;
      await this.buildSopa();
      this.empezarJuego();
    },
    ...
    buildSopa: async function () {
      this.enBuild = 1;
      this.timefin = 0;
      this.prepalabras = [];
      this.palabras = [];
      let tam = this.tam;
      let validPalabras = (await this.allpalabras()).filter(function (p) {
        return p.length < tam && p.length > Math.ceil(tam / 3 - 1)
          ? true
          : false;
      });
      ...
      for (let i = 0; i < tamax; i++) {
        rdm =
          Math.ceil(Math.random() * validPalabras.length) %
          validPalabras.length;
    ...
    setPalabras: function () {
      ...
      for (let ip in this.palabras) {
              ...
              for (let i = -1; i < 2; i++) {
                ...
                for (let l = 0; l < this.palabras[ip].p.length; l++) {
                  ...
                  if (tx >= 0 && tx < this.tam && ty >= 0 && ty < this.tam) {
                    if (
                      this.sopa[ty][tx].l != "" &&
                      this.sopa[ty][tx].l != this.palabras[ip].p[l]
                    )
                      ok = false;
                    else if (this.sopa[ty][tx].l == this.palabras[ip].p[l])
                      cz++;
                  } else {
                    ok = false;
                  }
        ...
        this.palabras[ip].o.sort((oa, ob) =>
          oa.a.y * this.tam + oa.a.x < ob.a.y * this.tam + ob.a.x ? -1 : 1
        );
        if (this.palabras[ip].o.length) {
          ...
          if (!f.length || ip % t == 1)
            f = this.palabras[ip].o.filter((op) =>
              Math.abs(op.c.y) == 0 ? true : false
            );
          if (!f.length || ip % t == 2)
            f = this.palabras[ip].o.filter((op) =>
              Math.abs(op.c.x) == 0 ? true : false
            );
          if (!f.length || ip % t == 3)
            f = this.palabras[ip].o.filter((op) =>
              Math.abs(op.c.x) == Math.abs(op.c.y) ? true : false
            );
          if (!f.length || ip % t == 4)
            f = this.palabras[ip].o.filter((op) => (op.z ? true : false));
          if (f.length) {
    ...
    putPalabra: function (idx, r) {
      ...
      this.palabras[idx].h = `rgb(${Math.random() * 96 + 96},${
        Math.random() * 96 + 96
      },${Math.random() * 96 + 96})`;
      this.palabras[idx].o = [];
    },
    fillVacios: function () {
      for (let y = 0; y < this.tam; y++) {
        for (let x = 0; x < this.tam; x++) {
          if (this.sopa[y][x].l == "")
            this.sopa[y][x].l = this.alfab[
              Math.floor(Math.random() * this.alfab.length)
            ];
        }
      }
      this.enBuild = 0;
    },
    allpalabras: async function () {
      return await api
        .multipleRandom()
        .then((x) => (x.body ? x.body : []))
        .then((data) =>
          data.map((x) => x.Word).filter((x) => x && x.length > 3)
        );
    },
  },
  computed: {
    ...
    palabrasOk: function () {
      return this.palabras.filter(function (pp) {
        return pp.u;
      });
    },
  },
  beforeMount: function () {
    this.frases.sort(function () {
